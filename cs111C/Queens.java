...
public boolean placeQueens(int column) {
  // --------------------------------------------------
  // Place queens in columns of the board beginning// at the column specified.
  // Precondition:  Queens are placed correctly in// columns 1 thro coulumn-1.
  ...
  // solution existis for a queen anywhere in column// specified).
  //--------------------------------------------------
  if (column > BOARD_SIZE) {  
      return true;  // base case
        ...
        // end if 
          ...
          if (!queenPlaced) {      
            // backtrack:  remove queen placed earliers      // and try next square in column      
...
private void setQueen(int row, int column) {// --------------------------------------------------// Set a queen at square indicated by row and// column.// Precondition:  None.// Postcondition:  Sets the square on the board in a// given row and column to QUEEN.// --------------------------------------------------
  board[row-1][column-1] = QUEEN;} // end setQueen
  ...
  while (lower_dir_row>=0 && lower_dir_column>=0){
    if (board[lower_dir_row][lower_dir_column]==1){
      return true;
        ...
        lower_dir_row = lower_dir_row -1;lower_dir_column = lower_dir_column -1;
        }
      }// check upper diagnal
    ...
    while (upper_dir_row<BOARD_SIZE && upper_dir_column>=0){
        if(board[upper_dir_row][upper_dir_column] ==1){
          ...
          upper_dir_row = upper_dir_row +1;upper_dir_column = upper_dir_column -1;
          }
          }return false;
} // end isUnderAttack
} // end Queens
